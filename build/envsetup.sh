...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^blackiron_") ; then
        BLACKIRON_BUILD=$(echo -n $1 | sed -e 's/^blackiron_//g')
    else
        BLACKIRON_BUILD=
    fi
    export BLACKIRON_BUILD
...
function breakfast()
{
    target=$1
    local variant=$2
    source ${ANDROID_BUILD_TOP}/vendor/blackiron/vars/aosp_target_release
    ...
    if [ $# -eq 0 ]; then
        ...
        if [[ "$target" =~ -(user|userdebug|eng)$ ]]; then
            ...
            # This is probably just the Blackiron model name
            if [ -z "$variant" ]; then
                variant="user"
            fi

            lunch blackiron_$target-$aosp_target_release-$variant
        fi
    fi
    return $?
}
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/BlackironProject-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop ro.blackiron.device | grep -q "$BLACKIRON_BUILD"); then
...
function githubremote()
{
    ...
    git remote add github https://github.com/BlackironProject/$PROJECT
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.blackiron.device | grep -q "$BLACKIRON_BUILD");
    then
...
function dopush()
{
    ...
    if (adb shell getprop ro.blackiron.device | grep -q "$BLACKIRON_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
